...
import { DaisyBook, NAV_LEVEL_PHRASE, NAV_LEVEL_PAGE, NAV_LEVEL_BOOKMARK, NAV_LEVEL_INTERVAL } from './daisybook';
...
export class DaisyPlayer {
    ...
    async loadStatus() {                                    
        ...
        try {
            const result = await this.file.checkFile(bdir, STATUS_FILENAME);
            if (result) {
